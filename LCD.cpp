...
void LCD::drawbitmap(uint8_t x, uint8_t y, 
...
void LCD::drawstring(uint8_t x, uint8_t line, char *c) {
  while (c[0] != 0) {
    drawchar(x, line, c[0]);
    c++;
    x += 6; // 6 pixels wide
...
void LCD::drawstring_P(uint8_t x, uint8_t line, const char *str) {
  while (1) {
...
void  LCD::drawchar(uint8_t x, uint8_t line, char c) {
...
// filled rectangle
void LCD::fillrect(uint8_t x, uint8_t y, uint8_t w, uint8_t h,
		      uint8_t color) {
...
// draw a rectangle
void LCD::drawrect(uint8_t x, uint8_t y, uint8_t w, uint8_t h,
		      uint8_t color) {
  // stupidest version - just pixels - but fast with internal buffer!
  for (uint8_t i=x; i<x+w; i++) {
...
void LCD::drawcircle(uint8_t x0, uint8_t y0, uint8_t r,
			uint8_t color) {
...
void LCD::fillcircle(uint8_t x0, uint8_t y0, uint8_t r,
			uint8_t color) {
  updateBoundingBox(x0-r, y0-r, x0+r, y0+r);
...
void LCD::my_setpixel(uint8_t x, uint8_t y, uint8_t color) {
...
void LCD::setpixel(uint8_t x, uint8_t y, uint8_t color) {
  if ((x >= LCDWIDTH) || (y >= LCDHEIGHT))
    return;
...
uint8_t LCD::getpixel(uint8_t x, uint8_t y) {
  if ((x >= LCDWIDTH) || (y >= LCDHEIGHT))
    return 0;
...
inline void LCD::spiwrite(uint8_t c) {
...
void LCD::st7565_command(uint8_t c) {
  digitalWrite(a0, LOW);

  spiwrite(c);
}

void LCD::st7565_data(uint8_t c) {
  digitalWrite(a0, HIGH);

  spiwrite(c);
}
void LCD::st7565_set_brightness(uint8_t val) {
...
void LCD::setBacklight(int color)
{
	...
	{
	case BLUE:
		digitalWrite(backlight_blue, LOW);
		digitalWrite(backlight_green, HIGH);
		digitalWrite(backlight_red, HIGH);
		break;
	case GREEN:
		digitalWrite(backlight_green, LOW);
		digitalWrite(backlight_blue, HIGH);
		digitalWrite(backlight_red, HIGH);
		break;
	case RED:
		digitalWrite(backlight_red, LOW);
		digitalWrite(backlight_blue, HIGH);
		digitalWrite(backlight_green, HIGH);
		break;
	default:
...
void LCD::fadeToBlack(int number)
{
	...
	writeLine(0, 0, result.c_str());

}

void LCD::writeLine(int x, int y, char* text)
{
	drawstring(x, y, text);
}